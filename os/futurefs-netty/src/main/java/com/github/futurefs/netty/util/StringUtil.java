...
public class StringUtil {
	...
	public static String merge(Object[] strArray, String separatorChars) {
		StringBuffer buffer = new StringBuffer();

		int offset = strArray.length - 1;
		for (int i = 0; i < offset; i++) {
			buffer.append(strArray[i]).append(separatorChars);
		}
		buffer.append(strArray[offset]);
	...
	public static String nubmerToChinese(Integer number) {
		NumberText nt = NumberText.getInstance(NumberText.Lang.ChineseSimplified);

		return nt.getText(number);
	}

	public static void main(String[] args) {
		for(int i = 1; i <= 100; i++) {
			String str = StringUtil.nubmerToChinese(i);
			System.out.println(String.format("insert into chapter (`name`, `remark`, `create_time`, `update_time`) " +
											 "values ('第%s章节', '第%s章节', NOW(), NOW());", str, str));
		}
	}
}
...
abstract class NumberText {
	 ...
	 * {@link NumberText#getInstance(Lang)}.
	 */
